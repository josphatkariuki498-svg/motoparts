...
");
$revenueData = [];
while ($r = $dailyRevenue->fetch_assoc()) $revenueData[] = $r;

// Top categories by sales
$topCats = $db->query("
    SELECT c.category_name, COUNT(od.order_detail_id) as items_sold, SUM(od.subtotal) as revenue
...
");
$catData = [];
while ($r = $topCats->fetch_assoc()) $catData[] = $r;
...
<form class="flex gap-2 items-center mb-6" method="GET" style="flex-wrap:wrap;">
  ...
  <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-chart-bar"></i> Generate Report</button>
...
<div class="stats-grid mb-6">
  ...
  <div class="stat-card yellow">
    ...
    <div class="stat-value">KSh <?= $summary['total_orders']>0 ? number_format($summary['total_rev']/$summary['total_orders']) : '0' ?></div>
...
<div class="card">
  ...
  <div class="table-wrap">
    <table>
      <thead>
        <tr><th>Category</th><th>Items Sold</th><th>Revenue</th><th>% Share</th></tr>
      </thead>
      <tbody>
        <?php
        $totalCatRev = array_sum(array_column($catData,'revenue'));
        foreach ($catData as $cat): $pct = $totalCatRev>0 ? round($cat['revenue']/$totalCatRev*100,1) : 0;
        ?>
        <tr>
...
new Chart(document.getElementById('dailyChart'), {
  ...
  options: {
    ...
    scales: {
      y: { ticks: { callback: v => 'KSh '+Number(v/1000).toFixed(1)+'K' }, grid: { color: 'rgba(255,255,255,0.05)' } },
...
new Chart(document.getElementById('catChart'), {
  ...
  options: {
    ...
    plugins: { legend: { display: false } },
    scales: {
      x: { ticks: { callback: v => 'KSh '+Number(v/1000).toFixed(1)+'K' }, grid: { color: 'rgba(255,255,255,0.05)' } },
