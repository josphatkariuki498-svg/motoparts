...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $id = (int)$_POST['category_id'];
        // Check against parts table (the correct one)
        $_r = $db->query("SELECT COUNT(*) c FROM spare_parts WHERE category_id=$id");
        $count = $_r ? (int)$_r->fetch_assoc()['c'] : 0;
        if ($count > 0) {
...
// Join against parts table which has is_active
$categories = $db->query("
    SELECT c.*, COUNT(p.part_id) AS part_count
    FROM categories c
    LEFT JOIN parts p ON c.category_id = p.category_id AND p.is_active = 1
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;">
  ...
  <div class="card" style="padding:22px;position:relative;">
    ...
    <div class="flex gap-2">
      <button class="btn btn-outline btn-sm" onclick='editCat(<?= json_encode($cat) ?>)'>
      ...
      <a href="parts.php?category=<?= $cat['category_id'] ?>" class="btn btn-ghost btn-sm">
